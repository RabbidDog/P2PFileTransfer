...
public class ClientManager implements Runnable {
    ...
    private volatile ConcurrentHashMap<Long, DataRequest> pendingPackets;
    private volatile AtomicLong highestOffsetReceived = new AtomicLong(0);
    ...
    public ClientManager(InetAddress address, int clientPort, int identifier, Frame requestFrame, boolean isDownload) throws SocketException
    {
    ...
    public void run()
    {
        ...
        { System.out.println("Executed thread for listening to client IP: "+ clinetAddress.toString()+ " Port: "+clientPort + "Request for "+ fileManager.getFileName());
            ...
            ConcurrentHashMap<Long, byte[]> bufferedFileData = new ConcurrentHashMap<Long, byte[]>(8096);
            ...
            while(true){
                ...
                /*read data from file at one go*/
                    if(!bufferedFileData.containsKey(offset))
                    {
                        System.out.println("Offset not in buffer. Read from file");
                        fileManager.bufferedRead(offset, lengthDataToBeSend*5, 512, bufferedFileData);
                    }
        ...
        else
        {
            ...
            pendingPackets = new ConcurrentHashMap<Long, DataRequest>(64);
            incomingFrames= new LinkedBlockingQueue<Frame>(4096);
                ...
                public void run() {
                    ...
                    for(;;) //this forloop keep sending requests till termination is received for file size is reached
                    {
                        ...
                        if(currentOffset< fileSize)
                        {
                            ...
                            {
                                System.out.println("SendPacket: pending packet size zero. Sending new request");
                                try
                                {
                                    boolean isLessThanDefaultSize = ((fileSize - currentOffset) / defaultPacketSize) == 0 ;
                                    if(isLessThanDefaultSize)
                                    {
                                        ...
                                        sock.send(packet);
                                        pendingPackets.putIfAbsent(currentOffset, request);
                                        currentOffset = fileSize;
                                    }
                                    else
                                    {
                                        int remainingWindow = 32 - pendingPackets.size();
                                        long remainingPackets = ((fileSize - currentOffset) / defaultPacketSize) >=32 ? 32 : ((fileSize - currentOffset) / defaultPacketSize);
                                        ...
                                        for (int i = 0; i < packetsToSend; i++) {
                                            ...
                                            pendingPackets.putIfAbsent(currentOffset, request1);
                            ...
                            else
                            {
                                ...
                                try
                                {
                                    ...
                                    System.out.println("InterruptedException in Thread SendRequest");
                    ...
                    /*packet must be termination or Data Response*/
                            if(f instanceof DataResponse)
                            {
                                ...
                                if(response.identifier() == identifier)
                                {
                                    ...
                                    {
                                        System.out.println("received packet for offset: "+response.offset()+" was pending");
                                        boolean checkToShutExecutor = false;
                                        System.out.println("pendingPackets size before removal: "+pendingPackets.size());
                                        pendingPackets.remove(response.offset());
                                        System.out.println("pendingPackets size after removal: "+pendingPackets.size());
                                        ...
                                        {
                                            if(checkToShutExecutor && currentOffset == fileSize)
                                            {
                                        ...
                                        System.out.println("ProcessPacket: packet at offset: "+response.offset()+" already received. Ignoring this packet");
                        ...
                        else
                        {
                            ...
                            for (Long offset: keySet)
                            {
                                if(offset < highestOffsetReceived.get())
                                {
                                    try
                                    {
                                        DataRequest resendPack = pendingPackets.get(offset);
                                        if(null != resendPack)
                                        {
